...
class LoginService:
    ...
    async def generate_access_token(self, username: str) -> JSONResponse:
        is_user_exist  = await self.pg_login_repo.is_user_exist(username)
    ...
    async def revoke_token(self, access_token: str) -> JSONResponse:
        revoked = await self.pg_login_repo.revoke_token(access_token)
        if not revoked:
    ...
    async def refresh_token(self, guid: str, refresh_token: str) -> JSONResponse:
        username  = await self.pg_login_repo.refresh_token(guid, refresh_token)
        if not username:
